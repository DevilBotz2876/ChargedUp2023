...
public class DockAndEngage extends SequentialCommandGroup {
   ...
   */
  public DockAndEngage(Drive drive, double maxDistance) {
    super();
    addCommands(CommandDebug.start());
    addCommands(new DriveStraightToDock(drive, maxDistance));
    addCommands(new BalancePID(drive));
    addCommands(new RotateDegrees(drive, 90));
    addCommands(CommandDebug.end());
  }

  /**
   * Similar to {@link #DockAndEngage(Drive, double)}, with the following difference(s):
   *
   * <ol>
   ...
   */
  public DockAndEngage(Drive drive, double maxDistance, double targetAngle) {
    super();
    addCommands(CommandDebug.start());
    addCommands(new DriveStraightToDock(drive, maxDistance, targetAngle));
    addCommands(new BalancePID(drive, targetAngle));
    addCommands(new RotateDegrees(drive, 90));
    addCommands(CommandDebug.end());
  }
}
